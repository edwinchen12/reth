//! Module that interacts with Redis.



use crate::{lockfile::StorageLock, metrics::DatabaseEnvMetrics, tables::{self, TableType, Tables}, utils::default_page_size, DatabaseError};
use eyre::Context;
...
/// Environment used when opening a redis connection. RO/RW.
#[derive(Debug)]
pub enum DatabaseKind {
    /// Read-only Redis connection.
    RO,
    /// Read-write Redis connection.
    RW,
}
...
pub struct DatabaseEnv {
    ...
    /// Readonly mode.
    /// If true, the connection is read-only.
    /// If false, the connection is read-write.
    readonly: bool,
}
...
impl DatabaseMetrics for DatabaseEnv {
    ...
    fn gauge_metrics(&self) -> Vec<(&'static str, f64, Vec<Label>)> {
        ...
        //TDOO: add redis stats using INFO
...
impl DatabaseMetadata for DatabaseEnv {
    ...
    fn metadata(&self) -> DatabaseMetadataValue {
        DatabaseMetadataValue::new(self.inner.freelist().ok())
    }
}

impl DatabaseEnv {
    /// Opens the database at the specified path with the given `RedisDatabaseKind`.
    ///
    /// It does not create the tables, for that call [`crate::DatabaseEnv::create_tables`].
    pub fn open(
        url: &str,
        path: &Path,
        kind: crate::implementation::redis::DatabaseKind,
        ...
        let is_readonly = !kind.is_rw();

        let mut inner_env = Environment::builder();

        let mode = match kind {
            DatabaseKind::RO => Mode::ReadOnly,
            DatabaseKind::RW => {
        ...
        inner_env.set_geometry(Geometry {
            ...
            size: Some(0..(4 * crate::implementation::redis::TERABYTE)),
            // We grow the database in increments of 4 gigabytes
            growth_step: Some(4 * crate::implementation::redis::GIGABYTE as isize),
        ...
        #[cfg(not(windows))]
        {
            ...
            inner_env.set_handle_slow_readers(
                |process_id: u32, thread_id: u32, read_txn_id: u64, gap: usize, space: usize, retry: isize| {
                    if space > crate::implementation::redis::MAX_SAFE_READER_SPACE {
        ...
        inner_env.set_max_readers(crate::implementation::redis::DEFAULT_MAX_READERS);
...
mod tests {
    ...
    use testcontainers::{core::{IntoContainerPort, WaitFor}, Container, GenericImage, runners::SyncRunner};

    struct DatabaseEnvTest {
        db: Arc<DatabaseEnv>,
    ...
    fn create_test_db(kind: DatabaseKind) -> DatabaseEnvTest {
    ...
    fn create_test_db_with_path(kind: DatabaseKind, path: &Path) -> DatabaseEnvTest {
        ...
        let db = DatabaseEnv::open(connection_info.as_ref(), path, DatabaseKind::RW, args)
            .expect(ERROR_DB_CREATION);
        db.create_tables().expect(ERROR_TABLE_CREATION);

        DatabaseEnvTest {
            db: Arc::new(db),
            redis: container,
            redis_url: connection_info,
        }
    }
    ...
    #[test]
    fn db_creation() {
        create_test_db(DatabaseKind::RW);
    }

    #[test]
    fn db_manual_put_get() {
        let test = create_test_db(DatabaseKind::RW);
        let env = &test.db;
    ...
    fn db_dup_cursor_delete_first() {
        let test = create_test_db(DatabaseKind::RW);
        let db: Arc<DatabaseEnv> = test.db;
    ...
    fn db_closure_put_get() {
        ...
        let test = create_test_db_with_path(DatabaseKind::RW, &path);
        {
            let env = test.db;
