...
impl DatabaseMetrics for DatabaseEnv {
    ...
    fn gauge_metrics(&self) -> Vec<(&'static str, f64, Vec<Label>)> {
        ...
        // TODO: add redis stats using INFO
        // let _ = self
        //     .view(|tx| {
...
mod tests {
    ...
    fn create_test_db(kind: DatabaseEnvKind) ->DatabaseEnvTest {
    ...
    fn create_test_db_with_path(kind: DatabaseEnvKind, path: &Path) -> DatabaseEnvTest {
    ...
    fn db_manual_put_get() {
        let test = create_test_db(DatabaseEnvKind::RW);
        let env = &test.db;
    ...
    fn db_closure_put_get() {
        ...
        let test = create_test_db_with_path(DatabaseEnvKind::RW, &path);
        {
            let env = test.db;
